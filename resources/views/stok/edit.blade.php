            ...
            <form method="POST" action="{{ url('/stok/'.$stok->stok_id) }}" class="form-horizontal">
                        ...
                        <select class="form-control" id="supplier_id" name="supplier_id" required>
                            ...
                            @foreach($supplier as $item)
                                <option value="{{ $item->supplier_id }}" 
                                    {{ ($item->supplier_id == $stok->supplier_id) ? 'selected' : '' }}>
                                    {{ $item->supplier_nama }}
                                </option>
                            @endforeach
                        </select>
                        @error('supplier_id') <small class="form-text text-danger">{{ $message }}</small> @enderror
                    </div>
                </div>
                        ...
                        <select class="form-control" id="barang_id" name="barang_id" required>
                            ...
                            @foreach($barang as $item)
                                <option value="{{ $item->barang_id }}"
                                    {{ ($item->barang_id == $stok->barang_id) ? 'selected' : '' }}>
                                    {{ $item->barang_nama }}
                                </option>
                            @endforeach
                        </select>
                        @error('barang_id') <small class="form-text text-danger">{{ $message }}</small> @enderror
                    </div>
                </div>
                        ...
                        <select class="form-control" id="user_id" name="user_id" required>
                            ...
                            @foreach($user as $item)
                                <option value="{{ $item->user_id }}"
                                    {{ ($item->user_id == $stok->user_id) ? 'selected' : '' }}>
                                    {{ $item->nama }}
                                </option>
                            @endforeach
                        </select>
                        @error('user_id') <small class="form-text text-danger">{{ $message }}</small> @enderror
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-11">
                        ...
                        @error('stok_tanggal') <small class="form-text text-danger">{{ $message }}</small> @enderror
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-11">
                        ...
                        @error('stok_jumlah') <small class="form-text text-danger">{{ $message }}</small> @enderror
                    </div>
                </div>
